...
const create = async (req, res) => {
  try {
    var sql =
      "INSERT INTO order_payment_method (name, code, status) VALUES (:name, :code, :status)";
    var param = {
      name: req.body.name,
      code: req.body.code,
    ...
    logError("order_payment_method.creeate", err, res);
  }
};
...
const update = async (req, res) => {
  try {
    var sql =
      "UPDATE order_payment_method SET name =:name, code =:code, status =:status WHERE id =:id";
    var param = {
      id: req.body.id,
      name: req.body.name,
      code: req.body.code,
